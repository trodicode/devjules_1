document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!loginForm || !loginMessageArea || !submitButton) {
        ...
        if (loginMessageArea) { // Try to show an error if message area exists
            showMessage('Critical error: Login interface is not correctly loaded. Please contact support.', 'error');
        }
        return;
    }
    ...
    if (typeof window.getUserByEmail !== 'function' || typeof window.COLUMN_NAMES === 'undefined') {
        console.error('Airtable API functions (getUserByEmail) or COLUMN_NAMES are not available.');
        showMessage('Critical error: Cannot connect to authentication system. Please try again later.', 'error');
        submitButton.disabled = true; // Disable form if critical components are missing
        return;
    }
    ...
    loginForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            if (!(userRole === 'Administrateur' || userRole === 'Utilisateur') || loginMessageArea.textContent.includes('error')) {
